...
class CoverageGranularity(str, Enum):
    ...
    CALENDAR_DATE_PARTITIONED = "CALENDAR_DATE_PARTITIONED"
    TRADE_DATE_PARTITIONED = "TRADE_DATE_PARTITIONED"
...
def _official(
    ...
    *,
    ...
    coverage: CoverageGranularity = CoverageGranularity.TRADE_DATE_PARTITIONED,
    contract_version: str = "1.0",
) -> EndpointContract:
    return EndpointContract(
        ProviderId.TUSHARE_OFFICIAL.value, dataset, dataset, coverage, contract_version,
...
_OFFICIAL = (
    _official("stock_basic", 25, ("ts_code", "symbol", "name", "area", "industry", "market", "exchange", "curr_type", "list_status", "list_date", "delist_date"), ("ts_code",), points=2000, per_minute=50, rows=6000, update=OFFICIAL_NOT_STATED, query="four explicit L/D/P/G calls merged as one atomic current reference snapshot", optional=("exchange", "list_status", "fields"), pit="current reference snapshot; historical eligibility derives from list_date/delist_date only", dependencies=("all-A universe", "custom/index lifecycle", "research backtest lifecycle"), coverage=CoverageGranularity.GLOBAL_SNAPSHOT, contract_version="1.1"),
    _official("trade_cal", 26, ("exchange", "cal_date", "is_open", "pretrade_date"), ("exchange", "cal_date"), points=2000, query="one exchange date range, incrementally cached", optional=("exchange", "start_date", "end_date", "is_open", "fields"), pit="calendar known for requested date", dependencies=("all research dates", "warm-up", "rebalance calendar"), coverage=CoverageGranularity.CALENDAR_DATE_PARTITIONED),
    ...
    _official("index_weight", 96, ("index_code", "con_code", "trade_date", "weight"), ("index_code", "con_code", "trade_date"), points=2000, rows=OFFICIAL_NOT_STATED, update=OFFICIAL_NOT_STATED, query="one index monthly snapshot window", required=("index_code",), optional=("trade_date", "start_date", "end_date"), pit="use only a constituent snapshot effective on or before formation", dependencies=("historical index universe",), coverage=CoverageGranularity.MONTH_PARTITIONED),
    ...
    def __init__(self) -> None:
        ...
        for item in _OFFICIAL:
            ...
            proxy = EndpointContract(
                ...
                item.coverage_granularity, item.contract_version,
